...
export const MOVE_MAP: { [key: string]: number } = Object.fromEntries(policy_index.map((value, index) => [value, index]));

function mirrorMoveUCI(move: string): string {
    const MOVE_RE = /^([a-h])([1-8])([a-h])([1-8])([qrbn]?)$/;
    const m = MOVE_RE.exec(move);
    if (!m) {
        throw new Error("Invalid move format");
    }
    return `${m[1]}${9 - parseInt(m[2])}${m[3]}${9 - parseInt(m[4])}${m[5]}`;
}
...
export function policy2moves(board_: Chess, policy_tensor: any, softmax_temp: number = 1.61): { [key: string]: number } {
    ...
    for (let uci in retval) {
        retval[uci] = Math.exp((retval[uci] - max_p) / softmax_temp);
        total += retval[uci];
    }
